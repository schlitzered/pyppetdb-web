        ...
        <a
          :href="
            router.resolve({
              ...
              params: { space_id: route.params.space_id, cert_id: item.id }
            }).href
          "
          @click.left.prevent
        >
...
async function batchCertAction(action) {
  for (const id of tableSelected.value) {
    let url = `/api/v1/ca/spaces/${route.params.space_id}/certs/${id}`
...
function onRowClick(item, item_data) {
  router.push({
    name: 'CASpacesCertsCRUD',
    params: { space_id: route.params.space_id, cert_id: item_data.item.id }
  })
}
</script>
