...
const tableConfig = {
  apiEndpoint: '/api/v1/ca/authorities',
  routeName: 'CAAuthoritiesSearch',
  fields: ['id', 'cn', 'status', 'issuer'],
  searchFormSchema: [{ key: 'ca_id', type: 'string' }]
}
...
const tableHeaders = [
  { title: 'CA ID', key: 'id', sortable: true },
  { title: 'Common Name', key: 'cn', sortable: false },
  { title: 'Issuer', key: 'issuer', sortable: false },
...
function onRowClick(item, item_data) {
  router.push({
    name: 'CAAuthoritiesCRUD',
    params: { ca_id: item_data.item.id }
  })
}
</script>
