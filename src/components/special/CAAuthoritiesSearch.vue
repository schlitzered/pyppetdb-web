  ...
  <div class="p-4 flex flex-col gap-4">
        ...
        <AccordionContent>
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4 py-2">
            ...
            <div class="flex flex-col gap-1">
              ...
              <InputText
                id="parent_id"
                v-model="formSearchBy.parent_id"
                @input="debouncedSearch"
                class="p-inputtext-sm bg-zinc-50 dark:bg-zinc-800 border-zinc-300 dark:border-zinc-700 text-zinc-900 dark:text-zinc-50"
              />
            </div>
    ...
    >
      <DataTable
        ...
        data-key="id"
      >
        <Column field="id" header="CA ID" sortable>
          <template #body="slotProps">
            <router-link
              :to="{
                name: 'CAAuthoritiesCRUD',
                params: {
                  ca_id: slotProps.data.id
                }
              }"
        ...
        <Column field="issuer" header="Issuer" sortable>
          <template #body="slotProps">
            <span
              class="text-sm font-mono text-xs text-zinc-600 dark:text-zinc-400"
            >
              {{ slotProps.data.issuer || 'N/A' }}
            </span>
          </template>
        </Column>

        <Column field="internal" header="Type">
          <template #body="slotProps">
            <Tag
              :value="slotProps.data.internal ? 'Internal' : 'External'"
              :severity="slotProps.data.internal ? 'info' : 'secondary'"
            />
          </template>
        </Column>
...
const typeOptions = [
  { label: 'Internal Only', value: true },
  { label: 'External Only', value: false }
]
...
const canCreate = computed(() => {
  return auth.hasPermission(PERMISSIONS.CA.AUTHORITIES.CREATE)
})
