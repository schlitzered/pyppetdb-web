...
const formButtonDeleteShow = computed(() => {
  if (route.params.level_id === '_new') return false
  return loginData.hasPermission(PERMISSIONS.HIERA.LEVELS.DELETE)
})
const formButtonEditShow = computed(() => {
  if (route.params.level_id === '_new') return false
  return loginData.hasPermission(PERMISSIONS.HIERA.LEVELS.UPDATE)
})
