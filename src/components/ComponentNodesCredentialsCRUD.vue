...
function dialogDeleteEvent(action) {
  if (action === 'cancel') {
    ...
    dialogDeleteShow.value = false
    dialogDeleteMsg.value = ''
    let url = `/api/v1/nodes/${route.params.node}/credentials/${formData.id}`
    api.delete(url).then(() => {
      router.push({
...
function formDelete() {
  ...
  dialogDeleteMsg.value = `Are you sure you want to delete node ${route.params.node} credential ${route.params.credential}`
}
...
function formSubmit(event) {
  ...
  if (route.params.credential === '_new') {
    url = '/api/v1/nodes/' + route.params.node + '/credentials'
    method = 'post'
  }
  api.request(method, url, data).then((data) => {
    if (route.params.credential === '_new') {
      ...
      router.push({
        ...
        params: { node: route.params.node, credential: data.id }
      })
    } else {
      formDataReadOnly.value = true
      formGetData()
...
function formGetData() {
  if (route.params.credential === '_new') {
    ...
    api
      .get(
        `/api/v1/nodes/${route.params.node}/credentials/${route.params.credential}`
      )
      .then((data) => {
        if (data) {
          formData['id'] = data['id']
